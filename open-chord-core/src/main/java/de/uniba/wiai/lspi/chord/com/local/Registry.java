/***************************************************************************
 * * Registry.java * ------------------- * date : 16.08.2004 * copyright : (C) 2004-2008 Distributed and * Mobile Systems Group * Lehrstuhl fuer Praktische
...
/**
 * This class represents a registry for {@link ThreadEndpoint endpoints}that can be used to build up a chord network within the same JVM with help of Java
 * Threads. This Registry is a singleton so that there is only one instance of this in the JVM. A reference to the singleton can be obtained by invocation of
 * {@link #getRegistryInstance()}.
 *
 * @author sven
 * @version 1.0.5
 */
public final class Registry {
	...
	/**
	 * Get a reference to the {@link List}of proxies used by the node with <code>nodeName</code>.
	 *
	 * @param url
	 *            The url of the node.
	 * @return {@link List}of {@link ThreadProxy proxies}that are used by the node with <code>nodeName</code>. May return <code>null</code> if there are no
	 *         proxies in use by the node.
	 */
	...
	/**
	 * Removes the {@link ThreadProxy proxies} used by the node with <code>nodeName</code>.
	 *
	 * @param url
	...
	/**
	 * Register the given {@link ThreadEndpoint endpoint}so that it can be looked up via {@link #lookup(String)}.
	 *
	 * @param endpoint
	...
	/**
	 * This method looks up the {@link ThreadEndpoint endpoint}for the node with the given name. If no endpoint is found <code>null</code> is returned.
	 *
	 * @param url
	 *            The url of the node, for which the {@link ThreadEndpoint endpoint}is looked up.
	 * @return Reference to the {@link ThreadEndpoint endpoint}of node with name "<code>name</code>". May be <code>null</code> if no such reference exists.
	 */
	 ...
	 *
	 * @return Array of all registered {@link ThreadEndpoint endpoints}. If no endpoint is registered an array of length 0 is returned.
	 */
	public Map<URL, ThreadEndpoint> lookupAll() {
		return this.registeredEndpoints;
		/*
		 * synchronized (this.registeredEndpoints){ ThreadEndpoint[] endpoints = new ThreadEndpoint[registeredEndpoints.size()]; Iterator iterator =
		 * this.registeredEndpoints.values().iterator(); int index = 0; while (iterator.hasNext()) { endpoints[index] = (ThreadEndpoint)iterator.next();
		 * index++; } return endpoints; }
		 */
	}
