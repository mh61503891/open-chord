/***************************************************************************
 * * Logger.java * ------------------- * date : 26. Januar 2004, 18:32 * copyright : (C) 2004 Distributed and Mobile Systems Group * Lehrstuhl fuer Praktische
 * Informatik * Universitaet Bamberg * http://www.uni-bamberg.de/pi/ * email : {jens.bruhn|sven.kaffille}@uni-bamberg.de * * *
 ***************************************************************************/
...
/**
 * This class is the abstract class to be implemented by a logger that should be used within the framework. <br/>
 * A reference to a logger can be obtained by {@link #getLogger(java.lang.String)} or {@link #getLogger(java.lang.Class)}. <br/>
 * You can use a custom logger by extending this class and setting the system property de.uniba.wiai.lspi.util.logging.logger.class to the FQN of your logger
 * class. <br/>
 * {@link #getLogger(java.lang.String)} or {@link #getLogger(java.lang.Class)} then try to instantiate your logger. If that fails one of the standard loggers is
 * created. The standard logger is {@link Log4jLogger}. If log4j is not availalbe from the classpath {@link DummyLogger} is the standard logger. If you set the
 * system property de.uniba.wiai.lspi.util.logging.off to true {@link DummyLogger} is the standard logger. <br/>
 * When implementing a custom logger you have to consider that the logger may be transferred over the network when classes are passed as parameters to remote
 * methods. The custom logger must have a public constructor with {@link java.lang.String} as parameter.
 *
 * @author sven
 * @version 1.0.5
 */
public abstract class Logger implements java.io.Serializable {
	...
	/**
	 * Map containing instances of loggers. Key: name of the logger. Value: The logger itself.
	 */
		...
		if (logger != null) {
			...
			if (!logOff) {
				...
				try {
					...
					/*
					 * Exception occured during instantiation of custom logger or Log4jLogger. Create dummy logger.
					 */
