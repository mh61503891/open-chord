...
final class References {
	...
	final synchronized Node getClosestPrecedingNode(ID key) {
		...
		if (this.logger.isEnabledFor(DEBUG)) {
			this.logger.debug("Closest preceding node of ID " + key + " at node " + this.localID.toString() + " is " + closestNode.getNodeID() + " with closestNodeFT="
					+ (closestNodeFT == null ? "null" : "" + closestNodeFT.getNodeID()) + " and closestNodeSL=" + (closestNodeSL == null ? "null" : "" + closestNodeSL.getNodeID())
					+ " and predecessor (only if it precedes given ID)=" + (predecessorIfAppropriate == null ? "null" : "" + predecessorIfAppropriate.getNodeID()));
		}
		return closestNode;
	}

	/**
	 * Adds the given node reference to the finger table and successor list, if appropriate. The reference is NOT set as predecessor, even if is closer to this
	 * node. Therefore use {@link #addReferenceAsPredecessor(Node)}.
	 *
	 * @param newReference
	...
	final synchronized void addReference(Node newReference) {
		...
		if (debug) {
			this.logger
			.debug("Attempted to add reference " + newReference.getNodeID().toString() + " to finger table and successor list. Whether it fit " + "or not depends on those data structures.");
		}
	}

	/**
	 * Removes the given node reference from the finger table and the successor list. If the given reference is the current predecessor, the predecessor
	...
	/**
	 * Returns a formatted string of the IDs of all references stored on this node. This includes references in the finger table and successor list as well as
	 * the predecessor.
	 *
	...
	/**
	 * Sets the given reference as this node's predecessor. If the former value of this predecessor's node was <code>null</code> and if this reference is not
	 * used any more (eg. in finger table or successor list), the connection to it is closed.
	 *
	...
	final synchronized void setPredecessor(Node potentialPredecessor) {
		...
		if (!(potentialPredecessor.equals(this.predecessor))) {
			...
			if (formerPredecessor != null) {
				...
				/*
				 * The replicas, which are in the range between the old and the new predecessor, on the last successor of this node have to be removed if the
				 * successor list is full. => capacity of sl == length of sl.
				 */
	...
	/**
	 * Determines if the given reference is contained in any one data structure, ie. finger table, successor list, or predecessor reference.
	 *
	 * @param newReference
	...
	/**
	 * Adds the given node reference to the finger table and successor list AND sets it as new predecessor reference, if appropriate. Appropriateness is given
	 * if either the former predecessor reference was <code>null</code> or the new reference's ID is located between the old predecessor ID and this node's ID.
	 * Even if not appropriate as predecessor, the reference is added to finger table and successor list.
	 *
	 * @param potentialPredecessor
	...
	void addReferenceAsPredecessor(Node potentialPredecessor) {
		...
		if (this.predecessor == null || potentialPredecessor.getNodeID().isInInterval(this.predecessor.getNodeID(), this.localID)) {
			...
			/*
			 * If a new predecessor, better than the old one has arrived, we have to copy the entries, that are relevant for this new predecessor. This has to
			 * be done by the predecessor itself, NOT here. 18.06.2007. sven
			 */
	...
	/**
	 * Added by sven on 21.03.2006. This data strucutre is supposed to work with remote references therefore it must be instances of Proxy. This method is used
	 * in every method that adds a new reference to this to check that it is an instance of Proxy. -> TODO: Consider to change type of all Nodes within this
	 * data structure to type Proxy!!!!
	 *
	 * @param node
